namespace Vala.Math {
    ...
    public class BigInteger : GLib.Object {
         ...
         * @return Result.ok(normalized BigInteger), or
         *         Result.error(BigIntegerError.INVALID_ARGUMENT) when value is invalid decimal text.
         */
        public static Result<BigInteger, GLib.Error> of (string value) {
            var normalized = normalize (value);
            if (normalized.isError ()) {
                return Result.error<BigInteger, GLib.Error> (normalized.unwrapError ());
            }
            return Result.ok<BigInteger, GLib.Error> (
                new BigInteger.fromNormalized (normalized.unwrap ())
            );
        }
         ...
         *         Result.error(BigIntegerError.DIVISION_BY_ZERO) when divisor is zero.
         */
        public Result<BigInteger, GLib.Error> divide (BigInteger other) {
            string divisorAbs = absValue (other._value);
            if (divisorAbs == "0") {
                return Result.error<BigInteger, GLib.Error> (
                    new BigIntegerError.DIVISION_BY_ZERO ("division by zero")
                );
            }
            ...
            if (quotientAbs == "0") {
                return Result.ok<BigInteger, GLib.Error> (
                    new BigInteger.fromNormalized ("0")
                );
            }
            ...
            if (negative) {
                return Result.ok<BigInteger, GLib.Error> (
                    new BigInteger.fromNormalized ("-" + quotientAbs)
                );
            }
            return Result.ok<BigInteger, GLib.Error> (
                new BigInteger.fromNormalized (quotientAbs)
         ...
         *         Result.error(BigIntegerError.DIVISION_BY_ZERO) when divisor is zero.
         */
        public Result<BigInteger, GLib.Error> mod (BigInteger other) {
            string divisorAbs = absValue (other._value);
            if (divisorAbs == "0") {
                return Result.error<BigInteger, GLib.Error> (
                    new BigIntegerError.DIVISION_BY_ZERO ("division by zero")
            ...
            if (isNegative (_value)) {
                return Result.ok<BigInteger, GLib.Error> (
                    new BigInteger.fromNormalized ("-" + remainder)
                );
            }
            return Result.ok<BigInteger, GLib.Error> (
                new BigInteger.fromNormalized (remainder)
         ...
         *         Result.error(BigIntegerError.INVALID_ARGUMENT) when exponent is negative.
         */
        public Result<BigInteger, GLib.Error> pow (int exponent) {
            if (exponent < 0) {
                return Result.error<BigInteger, GLib.Error> (
                    new BigIntegerError.INVALID_ARGUMENT ("exponent must be non-negative")
                );
            }
            ...
            return Result.ok<BigInteger, GLib.Error> (result);
        }
        ...
        private static Result<string, GLib.Error> normalize (string value) {
            if (value.length == 0) {
                return Result.error<string, GLib.Error> (
                    new BigIntegerError.INVALID_ARGUMENT ("value must not be empty")
                );
            }
            ...
            if (index >= value.length) {
                return Result.error<string, GLib.Error> (
                    new BigIntegerError.INVALID_ARGUMENT ("invalid integer text")
                );
            }
            ...
            if (negative) {
                return Result.ok<string, GLib.Error> ("-" + absolute);
            }
            return Result.ok<string, GLib.Error> (absolute);
        }
